...
/**
 * The main class for the CS410J Phone Bill Project 3
 */
...
public class Project3 {
    ...
    public static void main(String[] args) {
        ...
        int numOptionArgs = 6; // The max number of argument positions that the available options could take up
        ...
        for(int i = 0; i < numOptionArgs; ++i) { // Loop through to find how many options are used so we know how many positions to offset the phone call information
            ...
            else if(args[i].equals("-pretty")) { // If user wants to write a pretty version of the bill to a file or the console
        ...
        if((caller && callee && start && end)) { // If it passes all formatting tests, and all variables are set
            ...
            if(prettyFileName.equals("") == false) { // User has provided a file to pretty print to
                PrettyPrinter newPrettyFile = new PrettyPrinter();
                newPrettyFile.setFileName(prettyFileName);
                newPrettyFile.dump(bill);
            }
        }
        else {
